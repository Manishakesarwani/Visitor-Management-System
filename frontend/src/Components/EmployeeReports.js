...
const EmployeeReports = () => {
    ...
    const {user} = UseAuthorizationContext();
        ...
        const handleReportDownload = async()=>{
            ...
            const response = await fetch(`${process.env.REACT_APP_RENDER_URL}/api/admin/employee/export`, {
                method: "GET",
                headers: {
            ...
            anchor_tag.setAttribute("download", "Employee.csv");
  ...
  return (
    <div className='reports'>
        <button type="button" onClick={handleReportDownload} disabled={isExporting}>
            {isExporting ? "Exporting Employee..." : "Export All Employee"}
        </button>
    </div>
  )
}
