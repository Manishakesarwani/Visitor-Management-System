...
const SecurityReport = () => {
    ...
    const {user} = UseAuthorizationContext();
        ...
        const handleReportDownload = async()=>{
            ...
            const response = await fetch(`${process.env.REACT_APP_RENDER_URL}/api/admin/security/export`, {
                method: "GET",
                headers: {
            ...
            anchor_tag.setAttribute("download", "Security.csv");
  ...
  return (
    <div className='reports'>
        <button type="button" onClick={handleReportDownload} disabled={isExporting}>
            {isExporting ? "Exporting Security..." : "Export All Security"}
        </button>
    </div>
  )
}
