...
export const UseCheckInOut = () =>{
    ...
    const getLogs = async() => {
        const response = await fetch(`${process.env.REACT_APP_RENDER_URL}/api/checkpass`, {
            method: "GET",
            headers: {
    ...
    const checkPassId = async(passId) => {
        ...
        const response = await fetch(`${process.env.REACT_APP_RENDER_URL}/api/checkpass/${passId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${user.token}`
